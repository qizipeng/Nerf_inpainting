...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    k_extract = ['rgb_map', 'disp_map', 'acc_map', 'normal_map']
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, normal_map, _ = render(H, W, K, chunk=chunk, c2w=c2w[:3,:4], **render_kwargs)
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    return rgb_map, disp_map, acc_map, weights, depth_map, normal_map
...
#     raw = run_network(pts)
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, normal_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)

    if N_importance > 0:

        rgb_map_0, disp_map_0, acc_map_0, normal_map_0 = rgb_map, disp_map, acc_map, normal_map
...
#         raw = run_network(pts, fn=run_fn)
    ...
    ret = {'rgb_map' : rgb_map, 'disp_map' : disp_map, 'acc_map' : acc_map, 'normal_map':normal_map}
    if retraw:
        ret['raw'] = raw['outputs']
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        rgb, disp, acc, normal_map, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays,
